...
whatis("Description: the Spack package manager configured to install in a user specified directory. Spack release version 0.19.2 with minor patches to some packages.. CPE 23.03 compilers and libraries.")

family( 'LUMI_SoftwareStack' )

local spackroot = "/appl/lumi/spack/23.03/0.19.2-user"
...
if mode() == "load" then
  ...
  local moduledir = userdir .. "/23.03/0.19.2/modules/tcl"
  if not isDir(moduledir) then
    LmodMessage("Creating the Spack modules directory " .. (userdir .. "/modules/tcl"))
    ok,_,_ = os.execute("mkdir -p " .. userdir .. "/23.03/0.19.2/modules/tcl")
    if not ok then
  ...
  local cachedir = userdir .. "/23.03/0.19.2/cache"
  if not isDir(cachedir) then
    LmodMessage("Creating the Spack cache directory " .. (userdir .. "/cache"))
    ok,_,_ = os.execute("mkdir -p " .. userdir .. "/23.03/0.19.2/cache")
    if not ok then
