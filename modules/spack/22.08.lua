...
local spackroot = "/appl/lumi/spack/22.08/0.18.1-user"
...
if mode() == "load" then
  ...
  if not isDir(moduledir) then
    ...
    ok,_,_ = os.execute("mkdir -p " .. userdir .. "/22.08/0.18.1/modules/tcl")
    if not ok then
  ...
  local cachedir = userdir .. "/22.08/0.18.1/cache"
  if not isDir(cachedir) then
    LmodMessage("Creating the Spack cache directory " .. (userdir .. "/cache"))
    ok,_,_ = os.execute("mkdir -p " .. userdir .. "/22.08/0.18.1/cache")
    if not ok then
