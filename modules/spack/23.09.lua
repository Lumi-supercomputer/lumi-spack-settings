...
whatis("Version: 23.09")
whatis("Keywords: Spack")
whatis("Description: the Spack package manager configured to install in a user specified directory. Spack release version 0.21.0 with minor patches and backported bugfixes. CPE 23.09 compilers and libraries.")

family( 'LUMI_SoftwareStack' )

local spackroot = "/appl/lumi/spack/23.09/0.21.0-user"
...
if mode() == "load" then
  ...
  local moduledir = userdir .. "/23.09/0.21.0/modules/tcl"
  if not isDir(moduledir) then
    LmodMessage("Creating the Spack modules directory " .. (userdir .. "/23.09/0.21.0/modules/tcl"))
    ok,_,_ = os.execute("mkdir -p " .. userdir .. "/23.09/0.21.0/modules/tcl")
    if not ok then
  ...
  local cachedir = userdir .. "/23.09/0.21.0/cache"
  if not isDir(cachedir) then
    LmodMessage("Creating the Spack cache directory " .. (userdir .. "/23.09/0.21.0/cache"))
    ok,_,_ = os.execute("mkdir -p " .. userdir .. "/23.09/0.21.0/cache")
    if not ok then
